...
# go mod init main
# go mod tidy
${CC} -c ${CFLAGS} ${INCLUDEPATH} -fPIC crout.c
if [ $? -ne 0 ]; then exit 1; fi
${CC} ${LIBPATH} -shared -o libcrout.so crout.o -lmpfi -lmpfr -lgmp -fopenmp
if [ $? -ne 0 ]; then exit 1; fi
...
${CC} ${LIBPATH} -o cmain cmain.o forkjoin.o -lcrout -fopenmp
